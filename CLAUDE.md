# Minethon 開發提示詞 — 交給 AI 的完整實作指引

---

## 專案定義

你正在開發 **minethon**：一套 Python-first 的 Mineflayer SDK。
...
minethon 採用嚴格的三層分離架構。在寫任何程式碼之前，務必確認你要寫的東西屬於哪一層：

### Layer 1：Private Bridge（`minethon/_bridge/`）
...
### Layer 2：Domain Model（`minethon/models/`）
...
### Layer 3：Public SDK（`minethon/bot.py`, `minethon/api/`）
...
minethon 公開 API 採用 **asyncio** 作為唯一的 async 模型：
...
```python
"""01_hello_bot.py — 最基本的 minethon bot"""
import asyncio
from minethon import Bot
from minethon.models.events import ChatEvent, SpawnEvent
...
import asyncio
from minethon import Bot
from minethon.models.events import SpawnEvent
